...
# ============================================================
# 4. 閒聊（招呼、確認它還活著之類，不查情報庫）
# ============================================================

def small_talk(message):
    """老闆只是打招呼或閒聊時，自然回一句，不去查情報庫。

    讓雪倫除了是情報員，也是個活的、會回話的對象，
    這樣老闆隨手講句話就知道它還醒著。
    """
    prompt = f"""你是 AI 情報分析員雪倫，一隻平常盯著各大 AI 公司動態的助理。
老闆剛剛對你說了句話，這句話比較像是打招呼或閒聊，不是在問具體的 AI 情報。
請你自然、簡短地回他一句，像個有點個性、講話直接不囉嗦的助理。

要求：
- 繁體中文，口語、簡短，一兩句就好，不要長篇大論。
- 不要客套、不要過度熱情。
- 可以順帶提一句你都在盯 AI 情報、有新東西會通知他，或他可以問你 AI 相關的事，但別每次都硬塞。
- 所有標點符號一律使用全形，嚴禁半形。
...
你的回覆（用全形標點）："""
    try:
        return _sonnet(prompt, max_tokens=300)
    except Exception as e:
        logger.error("閒聊失敗：%s", e)
        return "我在，盯著呢。有 AI 相關的事都可以問我。"
